...
const addGridAction = actionCreator<IGridForm>('GRIDS/ADD');

export const addGridReducer = (
    state: IPresetsStore = defaultState,
    action: Action
) => {
    if (isType(action, addGridAction)) {
        return {
            ...state,
            selectedPreset:
...
export const addGridFormValidator = (
    form: IGridForm
): {
    layout?: string;
